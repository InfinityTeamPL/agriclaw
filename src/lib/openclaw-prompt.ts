...
// Wspólne reguły agronomiczne obu silników agenta (OpenClaw i wbudowany v2).
// Nazwy narzędzi wstrzykiwane per silnik — treść merytoryczna JEDNA.
export function buildAgroDecisionRules(t: {
  ndvi: string;
  weather: string;
  history: string;
  sorCheck: string;
  ...
  return `1. Pytanie o konkretne pole → wywołaj \`${t.ndvi}\` + \`${t.weather}\` (i \`${t.history}\` gdy oceniasz trend) PRZED odpowiedzią.
...
3. Spadek NDVI > 0.12 → NIE diagnozuj automatycznie choroby. Najpierw ustal:
   ...
   ZANIM wymienisz konkretny środek — wywołaj \`${t.sorCheck}\`: środka o statusie "wycofany" lub bez zastosowania w uprawie NIE zalecaj; dawkę cytuj z rejestru (pole dose), nie z pamięci; dołącz link do etykiety (labelPage).
...
${t.whatsapp ? `\n5. drought_risk = "high" (z danych pogodowych) → wyślij WhatsApp przez \`${t.whatsapp}\` z konkretną radą.\n\n6. Dobra kondycja (NDVI zgodny z fazą, brak suszy) → nie wysyłaj alertu, po prostu potwierdź że wszystko OK.` : `\n5. Dobra kondycja (NDVI zgodny z fazą, brak suszy) → potwierdź, że wszystko OK, bez alarmowania.`}`;
}

export const AGRO_STYLE_GUIDE = `Dobry przykład (konkret + framing wsparcia decyzji): "Pole za stodołą (pszenica), NDVI 0.42 — spadek 0.16 w tydzień. Susza 5 dni, ET0 4 mm/dzień. Jeśli zdecydujesz się na oprysk antytranspirantem, dobre okno to jutro 5:30–9:00 (po 10:00 rośnie wiatr). Zabieg i dawkę potwierdź z etykietą środka i przepisami."
...
export const CROP_PL: Record<string, string> = {
  wheat: 'pszenica',
  corn: 'kukurydza',
  rapeseed: 'rzepak',
  barley: 'jęczmień',
...
${buildAgroDecisionRules({
  ndvi: 'agri-satellite.ndvi',
  weather: 'agri-weather.forecast',
  history: 'agri-fields.history',
  sorCheck: 'agri-sor.check(product, crop)',
  whatsapp: 'agri-notify.whatsapp',
})}
